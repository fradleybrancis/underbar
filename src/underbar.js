(function() {
  ...
  _.filter = function(collection, test) {
   var passesTest = [];
    _.each(collection, function(element, index, ray) { 
     if (test(element, index, ray) === true) {
       passesTest.push(element);
     }
   });
   return passesTest;
  };
  ...
  _.map = function(collection, iterator) {
    ...
    var mapped = [];
    _.each(collection, function(element, index, ray) {
      mapped.push(iterator(element, index, ray))
    })
    return mapped;
  };
  ...
  _.some = function(collection, iterator) {
    ...
    return _.reduce(collection, function(memo, item) {
      if (iterator(item) == true || memo == true || typeof(iterator(item)) === 'string') {
        return true;
      }
      return false;
    }, false);
  ...
  _.delay = function(func, wait) {
    ...
    let fix = Array.prototype.slice;

    let args = fix.call(arguments);
  ...
  _.shuffle = function(array) {
    ...
    let newRay = array.slice()
    
    let currentIndex = newRay.length - 1;
    let randomIndex = getRandomInt(currentIndex)
    let temp;

    while (currentIndex !== 0) {
      
      temp = newRay[currentIndex];
      newRay[currentIndex] = newRay[randomIndex];
      newRay[randomIndex] = temp;
